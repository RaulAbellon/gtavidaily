...
/**
 * Tarjeta de artículo **presentacional**.
 *
 * Recibe un `ArticleView` ya resuelto (sin importar `@/lib/data`), así que se
 * puede usar tanto desde componentes de servidor —el resto del sitio, a través
 * de `ArticleCard`— como desde componentes de cliente, que es lo que necesitan
 * `/noticias` y `/buscar` para filtrar y paginar en el navegador.
 *
 * El marcado es exactamente el de antes: `<Link>` real, `<img>` con medidas
 * declaradas y carga diferida salvo en las primeras tarjetas.
 */
export function ArticleCardView({
  ...
  const href = `/articulo/${view.slug}`;
  const published = view.dateLabel;
      ...
      <Link href={href} className="group flex w-full items-start gap-3 text-left">
        ...
        <span className="flex-1">
          <span
            ...
            style={{ color: view.categoryColor }}
          >
            {view.categoryName}
          </span>
          ...
          <span className="mt-1 block text-[11px] text-zinc-500">
            {published} · {view.readingTime} min
          </span>
        </span>
      </Link>
    );
        ...
        >
          ...
          <span className="flex flex-col justify-center py-2 pr-4">
            ...
            <span className="mt-2 flex items-center gap-3 text-[11px] text-zinc-500">
              <span>{view.authorName}</span>
              <span aria-hidden="true">·</span>
              <time dateTime={view.publishedAt}>{published}</time>
            </span>
          </span>
        </Link>
      </article>
        ...
        <Link href={href} className="block">
          <span className="relative block aspect-[16/10] w-full overflow-hidden sm:aspect-[16/9]">
            ...
            <span
              ...
              style={{ backgroundColor: view.categoryColor }}
            >
              {view.categoryName}
            </span>
            {view.trending && (
          ...
          <span className="absolute inset-x-0 bottom-0 block bg-gradient-to-t from-zinc-950 via-zinc-950/70 to-transparent p-4 pt-10 sm:p-6 sm:pt-16">
            ...
            <span className="mt-3 flex flex-wrap items-center gap-3 text-xs text-zinc-300">
              <span className="font-medium text-white">{view.authorName}</span>
              <span aria-hidden="true">·</span>
              <time dateTime={view.publishedAt}>{published}</time>
              <span aria-hidden="true">·</span>
              <span className="inline-flex items-center gap-1">
                <Clock className="h-3 w-3" aria-hidden="true" />
                {view.readingTime} min
              </span>
            </span>
          </span>
        </Link>
